...
from wenshu_dbtext import FEATURE_COUNT, FEATURE_SCHEMA_VERSION  # noqa: E402

MINIMUM_CONFIDENCE = 0.92
MINIMUM_SCORE_MARGIN = 0.18
AI_DISPLAY_NAME = "文枢"
AI_INTERNAL_NAME = "AFR DBText WenShu"
...
def main() -> int:
    ...
    try:
        ...
        print("install with: python -m pip install -r tools/WenShu/DbText/requirements.txt", file=sys.stderr)
        return 10
    ...
    booster_path = output_dir / "AFR.DBTextAI.Model.txt"
    model.booster_.save_model(str(booster_path))

    manifest = build_manifest(features_path, df, test_report)
    write_json(output_dir / "AFR.DBTextAI.ModelManifest.json", manifest)

    if not args.skip_onnx:
        export_onnx(model, output_dir / "AFR.DBTextAI.Model.onnx", FEATURE_COUNT, args.opset, manifest)
    ...
    write_json(
        output_dir / "training_summary.json",
        {
            ...
            "modelManifest": portable_path(output_dir / "AFR.DBTextAI.ModelManifest.json"),
            "onnxModel": portable_path(output_dir / "AFR.DBTextAI.Model.onnx"),
        },
    )
    return 0
...
def export_onnx(model, output_path: Path, feature_count: int, opset: int, manifest: dict) -> None:
    ...
    onnx_model.producer_name = "CADFontAutoReplace WenShu DBText training pipeline"
