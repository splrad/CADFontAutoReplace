...
.DESCRIPTION
    执行步骤：
      1. 以 Release 配置构建通用 AFR.AwsPatchAgent
      2. 以 Release 配置构建所有 AFR-ACAD20XX 项目
         - DLL 与 CAD 元数据 JSON 均保留在标准构建输出目录
           artifacts\bin\AFR-ACAD20XX\release\
      3. 校验每个插件的 Release DLL 与 CAD 元数据 JSON
      4. dotnet publish AFR.Deployer -> 自包含 .NET 10 单文件 EXE
         （已内置 .NET Desktop Runtime 所需组件；无需 Windows App Runtime 等外置依赖）
      5. 生成版本化发布资产到 artifacts\ReleaseAssets\
...
$AgentCsproj    = Join-Path $RepoRoot "src\AFR.AwsPatchAgent\AFR.AwsPatchAgent.csproj"
...
# ── Step 1：构建通用 AWS 离线补写 agent 与所有插件 DLL ────────────────────
# 仅在未指定 -SkipPluginBuild 时执行。
# 成功后，DLL/JSON 留在 artifacts\bin\AFR-ACAD20XX\release\ 标准输出目录。
if (-not $SkipPluginBuild) {
    Write-Step "构建 AFR.AwsPatchAgent (Release)"
    $agentOutput = dotnet build $AgentCsproj -c Release --nologo -v quiet 2>&1
    if ($LASTEXITCODE -ne 0) {
        Write-Fail "AFR.AwsPatchAgent 构建失败"
        Write-Host ($agentOutput | Out-String) -ForegroundColor DarkRed
        exit 1
    }
    Write-Ok "AFR.AwsPatchAgent 完成"
