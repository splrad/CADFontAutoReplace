...
- ✅ **支持 MText 与 DBText 文字修复**：MText 内联字体缺失会自动处理；DBText 单行文字由封闭式本地 AI 模型保守判断，高置信度才写回。
...
AFR 会在样式表替换和 MText 内联字体处理完成后，对 DBText 单行文字执行疑似异常门控。未检测到疑似乱码或异常时保持静默，不加载文枢决策模型；只有门控命中后，才生成候选、提取特征并交给内置本地 AI 模型评分。高置信度、候选差距明确且通过安全检查的结果才会写回图纸。
