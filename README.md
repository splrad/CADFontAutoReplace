...
- ✅ **支持 MText 与 DBText 文字修复**：MText 内联字体缺失会自动处理；DBText 单行文字可结合模型数据集和人工确认逐步修复。
...
如果命令行提示仍有未修复的 DBText 单行文字，可输入 `AFRDBTEXTLABEL`，选择对应文字并确认处理方式：

1. 写回正确文本；
2. 保持当前文本；
3. 标记为字体/字形问题。

确认结果会记录到本地 DBText 修复模型中，后续相同对象上下文可由模型自动处理。
...
| `AFRDBTEXTLABEL` | 选择 DBText 单行文字并进行人工确认，记录修复/保留/字形问题标签 |
...
- [DBText：模型修复与人工确认](docs/debugging/DBText-Repair-Model.md)
