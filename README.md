...
AFR 会在样式表替换和 MText 内联字体处理完成后，对 DBText 单行文字读取内存中的 native 解码证据。强信号来自 Hook 数据流中可证明 DBCS / code page family mismatch 的对象级或簇级证据，不来自“读取到的文字看起来像乱码”。未命中 Hook 强证据时保持静默，不加载文枢决策模型；命中后才生成候选、提取 v3 特征并交给内置本地 AI 模型选择修复或保持当前文本。
