...
- Release 构建默认从仓库的 `AFR.GlyphCore/models` 嵌入当前发布模型，不从仓库读取训练数据；私有替代模型可通过 MSBuild 属性覆盖。
...
当前发布 ONNX 模型和模型清单是最终构建输入，应提交 GitHub；训练数据、用户 DWG、候选模型、训练状态和验证报告属于开发者私有资产，不提交 GitHub。训练脚本和工作台源码位于 `AFR.GlyphCore/tools`，可以作为工具链代码进入仓库。
