...
LastFilerCodePageId: 0x27(GBK/CP936 observed)
...
这表示 DWG filer 声明为目标图纸所需的 CP936，但 AutoCAD 在 DBCS 字节解码点传入了另一个 code page family，Hook 已在反序列化期间把解码入参修正为 filer code page。单行文字图纸可能只命中 `TextEditor DBCS Decode Hook`，不命中 `Code Page Family Hook`。
...
- `LastCodePageId` 不是 `0x27`: 当前 DWG filer 没有提供目标图纸所需的 CP936 证据，不能在无误杀约束下按 CP936 修复。
