...
DBText 单行文字修复不再依赖读取出来的文字是否像乱码作为门控，也不再使用公开 JSONL 标签、本地训练或人工标注命令。当前实现使用 native DBCS/code page Hook evidence 强信号、封闭式本地 ONNX AI 模型、确定性候选、v3 特征、簇级判断和涟漪上下文。
...
- [ ] DBText AI 模型缺失、schema 不匹配、无 Hook 强证据静默、AI 选择 no-op 和 AI 选择写回路径已验证
