...
普通样式表检测和永久写回应优先使用 AutoCAD 托管 API，例如当前 `Database` 上的 `HostApplicationServices.Current.FindFile`；`FontAvailabilityIndex` 只是 native Hook 路径无法安全取得托管 `Database` 时的进程级兜底索引。
...
这些 Hook 只生产文枢强信号证据，不直接修正文本文字、不直接改 native code page。Hook 安装由各 `AutoCad20XXPlatform` 暴露的 `NativeDecodeHookProfile` 决定；当前静态基线完整启用 AutoCAD 2022-2027，2027 的 `readString(wchar**)` 通过 RTTI/vtable 静态确认的 RVA 安装并在运行时校验入口 prefix，AcPal UTF16 probe 通过 2024-2027 的 `acpal#*.dll` 基线启用。AutoCAD 2018-2021 虽可经 RTTI/vtable 定位 `AcDbImpText::dwgInFields`，但缺少完整 readString/code-page resolver 验证，必须 fail closed。
...
不要恢复改写 `MText.Contents` 的旧转换器；当前策略是扫描内容、触发 CAD 原生展开流程、由 Hook 记录真实运行时映射。
...
DBText 单行文字修复不再依赖读取出来的文字是否像乱码作为门控，也不再使用公开 JSONL 标签、本地训练或人工标注命令。当前实现使用 native DBCS/code page Hook evidence 强信号、封闭式本地 ONNX AI 模型、确定性候选、`dbtext-ai-features-v7` 特征、簇级判断和涟漪/同文档上下文。
...
- `GlyphCoreTextRepairFeatureExtractor` 必须保持跨 CAD 版本和用户环境稳定：字体文件名、BigFont 文件名、typeface、字体族和 `LdFileHook` 字体证据特征槽位保留但置零，DBText 修复不能靠字体文件名决策。
- Decision Engine 默认以 AI 在 no-op 与修复候选之间的选择为准；只有 native evidence 明确、候选方向匹配、可逆且文本安全时，才允许低置信度、小 margin 或 `current-noop` 微弱领先的保守接受路径。
- 导出、工作台和训练必须保留 CAD 当前实际文本；不要恢复 `井/#` 显示别名归一化，也不要写死某个图纸里的 `井 -> #` 或 `# -> 井` 规则。
- 普通用户端禁止训练、导入样本、替换模型、修改参数或使用 DBText 纠错 UI。
- 当前发布 ONNX 模型和模型清单是最终构建输入，应提交 GitHub；训练数据、用户 DWG、候选模型、训练状态和验证报告属于开发者私有资产，不提交 GitHub。工具、schema、训练脚本和工作台源码位于 `AFR.GlyphCore/tools`。
...
- `MTextInlineFontHook`：看 inline scope、setter/loadStyleRec 命中和 `FontRuntimeMappingStore` 记录；
- `ExecutionController.Execute` 样式表与 MText 两个触发型 `Regen()`：核对 Hook 命中顺序，不要与最终视觉刷新随意合并。
...
官方文枢 GlyphCore 模型默认从仓库的 `AFR.GlyphCore/models` 自动嵌入。需要覆盖为私有替代模型时，可额外传入模型、清单和 ONNX Runtime 目录：
