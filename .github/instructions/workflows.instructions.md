...
- `Main Authorization Gate` job `name` 和自定义 Checks API required check `Copilot Code Review Gate` 是否被改动；本仓库 ruleset 依赖这两个 check 名称，且 `Copilot Code Review Gate` 的 required check source 必须匹配创建 check run 的 GitHub App 或 Any source。
