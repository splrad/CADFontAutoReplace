...
/// <summary>
/// DBCS Code Page Family 修复 Hook。
/// <para>
/// 该类型仅在 DEBUG 构建中编译。它通过运行时唯一签名定位 code-page context 初始化函数，
/// 在 AutoCAD 使用 session-wide 系统 code page 初始化文本解码上下文后，
/// 根据当前 DWG filer 的真实 DBCS code page 覆写 <c>[context+0x46C]</c>。
/// </para>
/// </summary>
internal static class CodePageFamilyHook
{
    ...
    public static void Uninstall()
    {
        ...
        _installed = false;
        DiagnosticLogger.Log(Tag, $"已卸载。HitCount={_hitCount}, PatchedCount={_patchedCount}, NoScope={_noScopeCount}");
    }
    ...
    private static void HookHandler(IntPtr context, int arg2)
    {
        ...
        try
        {
            ...
            if (!DwgFilerCodePageScopeHook.TryGetCurrentDbcsCodePageId(out int filerCodePageId))
            {
                ...
                {
                    DiagnosticLogger.Log(Tag,
                        $"未修正: return=0x{_lastReturnRva:X}, context=0x{context.ToInt64():X}, " +
            ...
            Marshal.WriteInt32(fieldAddr, filerCodePageId);
            _lastPatchedCodePageId = filerCodePageId;
            Interlocked.Increment(ref _patchedCount);

            DiagnosticLogger.Log(Tag,
                $"已修正: return=0x{_lastReturnRva:X}, context=0x{context.ToInt64():X}, " +
