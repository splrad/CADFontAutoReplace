...
internal static class GlyphCoreTextRepairProblemDetector
{
    public static GlyphCoreTextRepairProblemDetection Detect(GlyphCoreTextRepairContext context)
    {
        ...
        IReadOnlyList<GlyphCoreTextRepairCandidate> candidates = GlyphCoreTextRepairCandidateGenerator.BuildCandidates(current);
        if (string.IsNullOrWhiteSpace(current))
            return GlyphCoreTextRepairProblemDetection.NoProblem(candidates);

        if (GlyphCoreTextRepairFeatureExtractor.HasUnsafeText(current))
            return GlyphCoreTextRepairProblemDetection.Problem("unsafe-current-text", candidates);

        TextStats currentStats = Analyze(current);
        if (LooksLikeMojibake(current, currentStats))
            return GlyphCoreTextRepairProblemDetection.Problem("mojibake-pattern", candidates);

        for (int i = 0; i < candidates.Count; i++)
        {
            GlyphCoreTextRepairCandidate candidate = candidates[i];
            if (candidate.IsNoOp || !candidate.IsRoundTrip || GlyphCoreTextRepairFeatureExtractor.HasUnsafeText(candidate.Text))
                continue;
            ...
            if (sourceLooksRelevant && (cjkImproved || cadTermsImproved))
                return GlyphCoreTextRepairProblemDetection.Problem("roundtrip-candidate-improved", candidates);
        }
